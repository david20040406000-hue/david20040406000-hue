...
<desc id="desc">Zhaxia 的個人首頁。自主決策與演算法；作品：Screeps Empire AI；任務調度、空間規劃與 CPU 預算管理。軌道與光線為裝飾動畫。</desc>
<defs>
...
<style>
  ...
  .orbit {{transform-box:fill-box;transform-origin:center;animation:rotate 24s linear infinite;}}
  .orbit-back {{animation-direction:reverse;animation-duration:36s;}}
  ...
  @media (prefers-reduced-motion: reduce) {{ .reveal,.type-reveal,.orbit,.flow,.cursor {{animation:none !important;}} }}
</style>
...
''']
    ...
    if not mobile:
        out.append(f'''<g transform="translate(780 164)">
<circle r="101" fill="none" stroke="{c['line']}"/>
<circle r="75" fill="none" stroke="{c['line']}"/>
<path d="M-122 0H122M0-122V122" stroke="{c['line']}" stroke-dasharray="3 7"/>
<g class="orbit"><circle r="101" fill="none" stroke="{c['accent']}" stroke-width="2" stroke-dasharray="65 570"/><circle cx="101" r="4" fill="{c['accent']}"/></g>
<g class="orbit orbit-back"><circle r="75" fill="none" stroke="{c['blue']}" stroke-width="2" stroke-dasharray="45 427"/><circle cx="-75" r="3" fill="{c['blue']}"/></g>
<path d="M-28-30H30L-30 30H28" fill="none" stroke="url(#accent)" stroke-width="9" stroke-linejoin="round"/>
</g>''')
